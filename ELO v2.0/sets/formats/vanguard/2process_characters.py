...
def main():
    ...
    # Characters in the inactive list that never appeared in the input data get a 🐐 placeholder
    retired_names = [name for name in INACTIVE_CHARACTERS if name not in all_records]
    if retired_names:
        print(f"Retired (no data) characters: {retired_names}")

    active_output = {"tableData": build_table_data(active_records)}

    inactive_table = build_table_data(inactive_records)
    ...
    inactive_output = {"tableData": inactive_table}
